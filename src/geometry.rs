...
impl Square {
    ...
    pub fn corners(&self) -> [Point; 4] {
        ...
        corners
            .map(|p| p.rotate_around(&Point::new(0.0, 0.0), self.angle))
    ...
    fn sat_overlap(&self, other: &Square) -> bool {
        ...
        for corners in [&corners1, &corners2] {
            for i in 0..4 {
                ...
                let proj1: Vec<f64> = corners1
                    .iter()
                    .map(|c| normal.x * c.x + normal.y * c.y)
                    .collect();
                let proj2: Vec<f64> = corners2
                    .iter()
                    .map(|c| normal.x * c.x + normal.y * c.y)
                    .collect();
    ...
    pub fn is_inside_container(&self, container_size: f64) -> bool {
        if self.angle == 0.0 {
            ...
            self.corners().iter().all(|corner| {
                corner.x >= 0.0
                    && corner.x <= container_size
                    && corner.y >= 0.0
                    && corner.y <= container_size
            })
        }
    }
...
mod tests {
    ...
    fn test_rotated_square_corners() {
        ...
        let distances: Vec<f64> = corners.iter().map(|c| center.distance_to(c)).collect();
